...
if(isset($_POST['submit']))
{
	...
	if(isset($_GET['editid']))
	{
		...
		$qty = $_POST['quantity'];
		$sql ="UPDATE product SET  seller_id='$_SESSION[sellerid]', category_id='$_POST[category]', produce_id='$_POST[produce]', title='$_POST[title]', ";
		if($_FILES['img1']['name'] != "")
		{
	...
	{
$sql="INSERT INTO product(seller_id, category_id, produce_id,  title, img_1";
if($_FILES['img2']['name'] != "")
{
$sql = $sql . " , img_2";
}
$sql = $sql . " , quantity, quantity_type, description, uploaded_date, status) VALUES ('$_SESSION[sellerid]','$_POST[category]','$_POST[produce]','$_POST[title]','$imgname1'";
if($_FILES['img2']['name'] != "")
{
$sql = $sql . " ,'$imgname2'";
}
...
<div class="form-row">
	...
	Quantity Type <font color="#FF0000">*</font>
	 <select name="quantitytype" id="quantitytype" class="form-control">
                                 <option value="">Select</option>
                                  <?php
								  $arr= array("Kilogram","Gram","Quintal","No:s");
								  foreach($arr as $val)
								  {
